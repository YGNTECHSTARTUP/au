...
export default function HackathonForm() {
    ...
    const updateEmail = authstore((state) => state.updateEmail);
    
    const phoneNumber = authstore((state) => state.phoneNumber);
    const updatePhoneNumber = authstore((state) => state.updatePhoneNumber);
    
    const gender = authstore((state) => state.gender);
    const updateGender = authstore((state) => state.updateGender);
    
    const college = authstore((state) => state.college);
    const updateCollege = authstore((state) => state.updateCollege);
    
    const course = authstore((state) => state.course);
    const updateCourse = authstore((state) => state.updateCourse);
    
    const branch = authstore((state) => state.branch);
    const updateBranch = authstore((state) => state.updateBranch);
    
    const yearOfStudy = authstore((state) => state.yearOfStudy);
    const updateYearOfStudy = authstore((state) => state.updateYearOfStudy);
    
    const city = authstore((state) => state.city);
    const updateCity = authstore((state) => state.updateCity);
    
    const stateName = authstore((state) => state.state);
    const updateState = authstore((state) => state.updateState);
    
    const theme = authstore((state) => state.theme);
    const updateTheme = authstore((state) => state.updateTheme);
    
    const teamSize = authstore((state) => state.teamSize);
    const updateTeamSize = authstore((state) => state.updateTeamSize);
    
    const teamName = authstore((state) => state.teamName);
    const updateTeamName = authstore((state) => state.updateTeamName);
    
    const paymentScreenshot = authstore((state) => state.paymentScreenshot);
    const updatePaymentScreenshot = authstore((state) => state.updatePaymentScreenshot);
    
    const isAgreed = authstore((state) => state.isAgreed);
    const updateAgreement = authstore((state) => state.updateAgreement);
  ...
  function onSubmit(values: z.infer<typeof formSchema>) {
     updateName(values.fullName)
  updateEmail(values.email)
  updateCollege(values.organization)
  updateYearOfStudy(values.yearBranch)
  updateTeamSize(values.teamSize)
  setStep(2);
  }
